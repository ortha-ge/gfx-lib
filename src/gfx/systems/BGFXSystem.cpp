...
namespace Gfx {
	...
	void BGFXSystem::tickSystem(entt::registry& registry) {
		...
		auto createShaderView = registry.view<const ShaderDescriptor, const Core::RawDataResource>(
				entt::exclude<Core::FileLoadRequest, BGFXShader>);
		createShaderView.each(
				[&registry, &bgfxContext](entt::entity entity, const Core::RawDataResource& rawDataResource) {
		...
		createProgramView.each([&registry, &bgfxContext](entt::entity entity,
														 const ShaderProgramResource& shaderProgramDescriptor) {
			...
			const BGFXShader* fragmentShader =
					registry.try_get<BGFXShader>(shaderProgramDescriptor.fragmentShaderResource);
		...
		createTexturesView.each(
				[&registry, &bgfxContext](entt::entity entity, const Core::ImageResource& imageResource) {
					const bgfx::Memory* mem = bgfx::copy(imageResource.image.data(), imageResource.image.size());
					bgfx::TextureHandle texture = bgfx::createTexture2D(
							imageResource.width, imageResource.height, imageResource.numMips > 1,
							imageResource.numLayers, static_cast<bgfx::TextureFormat::Enum>(imageResource.format),
		...
		renderObjectsView.each([&registry, &perMaterialTransientBuffers](entt::entity entity,
			...
			if (!perMaterialTransientBuffers.contains(materialEntity)) {
				...
				perMaterialTransientBuffers.emplace(materialEntity,
													QuadBufferData{transientVertexBuffer, transientIndexBuffer});
			}
			...
			PosColorVertex* vertexHead =
					reinterpret_cast<PosColorVertex*>(transientBuffer.transientVertexBuffer.data) + startVertex;
